...
const Navbar = () => {
    ...
    const [tocken, setTocken] = useState(true)
  ...
  return (
    <div className="flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400">
      ...
      <div className="flex items-center gap-4">
            ...
            ? <div className="flex items-center gap-2 cursor-pointer group relative">
                ...
                <div className="absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block">
                    <div className="min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4">
                        ...
                        <p onClick={() => setTocken(false)} className="hover:text-black cursor-pointer">Log Out</p>
                    </div>
                </div>
            </div>
            : <button
        ...
        <img 
        onClick={() => setShowMenu(true)} className="w-6 md:hidden ml-10" src={assets.menu_icon} alt="" />
        {/*  ---------mobile menu---------- */}
        <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'}md:hidden left-0 right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
